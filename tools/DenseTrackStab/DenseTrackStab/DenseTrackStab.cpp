// DenseTrackStab.cpp : Defines the entry point for the console application.
...
int main(int argc, char* argv[])
{
	...
	//std::cout << argv[1];
	int flag = false;  //arg_parse(argc, argv);
	capture.open(video);
			...
			for (std::list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end();) {
				...
				if (iTrack->index >= trackInfo.length) {
					...
					if (IsValid(trajectory, mean_x, mean_y, var_x, var_y, length) && IsCameraMotion(displacement)) {
						// output the trajectory -- 4 of %f -- trajectory start x,y - end x,y after frame_num 
						printf("%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t", frame_num, 
							iTrack->point[0].x, iTrack->point[0].y, // 3
							iTrack->point[1].x, iTrack->point[1].y, // 5
							iTrack->point[2].x, iTrack->point[2].y, // 7
							iTrack->point[3].x, iTrack->point[3].y, // 9
							iTrack->point[4].x, iTrack->point[4].y, // 11
							iTrack->point[5].x, iTrack->point[5].y, // 13
							iTrack->point[6].x, iTrack->point[6].y, // 15
							iTrack->point[7].x, iTrack->point[7].y, // 17
							iTrack->point[8].x, iTrack->point[8].y, // 19
							iTrack->point[9].x, iTrack->point[9].y, // 21
							iTrack->point[10].x, iTrack->point[10].y, // 23
							iTrack->point[11].x, iTrack->point[11].y, // 25
							iTrack->point[12].x, iTrack->point[12].y, // 27
							iTrack->point[13].x, iTrack->point[13].y, // 29
							iTrack->point[14].x, iTrack->point[14].y, // 31
							mean_x,  mean_y, var_x, var_y, length, fscales[iScale]); // 37
						...
						for (int i = 0; i < trackInfo.length; ++i) // 41,42
